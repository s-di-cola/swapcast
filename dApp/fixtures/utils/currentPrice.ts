...
import { makeApiRequest, buildEndpoint, buildQueryString } from '../../src/lib/services/price/client';
...
export async function getCurrentPrice(assetSymbol: string): Promise<number> {
    ...
    try {
        ...
        console.error(chalk.yellow(`Error fetching price for ${assetSymbol}, using fallback price:`), error);
