...
function createPriceFromMarketData(
    ...
    const baseConfig = TOKEN_CONFIGS[baseToken.symbol];
    const quoteConfig = TOKEN_CONFIGS[quoteToken.symbol];
    
    if (!baseConfig || !quoteConfig) {
        throw new Error(`Token config missing for ${baseToken.symbol} or ${quoteToken.symbol}`);
    }
