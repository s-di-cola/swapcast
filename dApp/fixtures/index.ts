...
async function checkAnvilAndContracts() {
	try {
		...
		try {
			...
			console.error(
				chalk.red(
					'❌ Required contracts are not deployed or not accessible. Please run the deployment script first.'
				)
			);
...
async function main() {
	...
	try {
		...
		console.log(
			chalk.green(`✅ Total accounts available for predictions: ${allPredictionAccounts.length}`)
		);
		...
		for (let batchIndex = 0; batchIndex < marketBatches.length; batchIndex++) {
			...
			console.log(
				chalk.cyan(
					`📊 Processing batch ${batchIndex + 1}/${marketBatches.length} (${batch.length} markets)...`
				)
			);
