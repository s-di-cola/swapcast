...
export async function generatePredictions(
	...
	try {
		...
		try {
			...
			console.log(
				chalk.green(`Market ${market.id} exists with pool key: ${JSON.stringify(poolKey)}`)
			);
		} catch (error) {
		...
		console.log(
			chalk.green(`Market ${market.id} exists and has not expired. Proceeding with predictions.`)
		);
	} catch (error) {
	...
	console.log(
		chalk.yellow(`Using ${actualCount} users for predictions (limited by available user accounts)`)
	);
	...
	for (let i = 0; i < actualCount; i++) {
		...
		try {
			...
			const convictionStake = await getRandomStakeAmount(
			...
			console.log(
				chalk.blue(
					`User ${userAccount.address} predicting outcome ${outcome === OUTCOME_BULLISH ? 'Bullish' : 'Bearish'}`
				)
			);
			...
			if ('privateKey' in userAccount && userAccount.privateKey) {
				...
				while (attempts < maxAttempts) {
					try {
						...
						console.log(
							chalk.yellow(
								`Attempt ${attempts}/${maxAttempts} failed for ${userAccount.address}. Retrying...`
							)
						);
						...
						if (err.message && err.message.includes('Insufficient funds')) {
							...
							await publicClient.request({
								...
								params: [
									userAccount.address,
									('0x' + (BigInt(2000) * BigInt(1e18)).toString(16)) as any
								] // 2000 ETH
							});
						}
			...
			console.error(
				chalk.red(`Failed to record prediction for user ${userAccount.address}:`),
				error.message
			);
