...
export async function generateMarkets(
	...
	for (let i = 0; i < count; i++) {
		...
		const marketSuffix =
			i >= ASSET_PAIRS.length ? ` #${Math.floor(i / ASSET_PAIRS.length) + 1}` : '';
		...
		try {
			...
			console.log(
				chalk.blue(`Using price threshold of $${priceThreshold.toFixed(2)} for ${assetPair.symbol}`)
			);
			...
			const expirationTime = BigInt(
				Math.floor(Date.now() / 1000) + daysToExpiration * 24 * 60 * 60
			);
