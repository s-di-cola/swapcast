...
test.describe('Admin Dashboard', () => {
  ...
  test('should load the admin dashboard and display market list', async ({ page }) => {
    ...
    try {
      ...
      const noMarketsMessage = await page.locator('text=No markets, text=No data, text=Empty').count();
      ...
      const marketContent = await page.locator('[data-testid*="market"], .market-item, .market-card').count();
      if (marketContent > 0) {
    ...
    for (const header of expectedHeaders) {
      const headerCount = await page.locator(`th:has-text("${header}"), [role="columnheader"]:has-text("${header}")`).count();
      if (headerCount > 0) {
        foundHeaders++;
      }
    }
  ...
  test('should navigate to market creation page', async ({ page }) => {
    ...
    try {
      await Promise.all([
        page.waitForNavigation({ timeout: 5000 }).catch(() => console.log('Navigation timeout, continuing')),
        createButton.click()
      ]);
    } catch (e) {
    ...
    try {
      const heading = page.locator('h1, h2, h3').filter({ hasText: /Create|New|Market|Prediction/i });
      await heading.waitFor({ state: 'visible', timeout: 5000 });
      expect(await heading.isVisible()).toBeTruthy();
    } catch (e) {
      // If we can't find a heading, look for any form elements that suggest we're on a creation page
      const formElements = await page.locator('form, #marketName, [name="marketName"], input[placeholder*="Market"]').count();
  ...
  test('should navigate to market details when clicking on a market', async ({ page }) => {
    ...
    try {
      await page.waitForSelector('table, .market-table, .market-list, [data-testid*="market-list"]', { state: 'visible', timeout: 5000 });
    } catch (e) {
    ...
    try {
      ...
      const headingSelectors = [
        'h1', 'h2', 'h3', 'h4', '.title', '.header', 
        '[data-testid*="title"]', '[data-testid*="header"]',
    ...
    if (!detailsFound) {
      try {
        const contentSelectors = [
          ...
          '.market-details', '.details-container', '[data-testid*="market-detail"]', 
          'dl', '.detail-item', '.market-info', '.market-data',
          
          // Generic content selectors
          'main', 'article', 'section', '.content', '.container',
          'table', '.card', '.panel', '.box'
        ];
    ...
    if (!detailsFound) {
      try {
        ...
        const detailsTerms = ['market', 'details', 'information', 'overview', 'status', 'price', 'pair', 'token', 'asset'];
