...
async function connectWallet(page: Page) {
  ...
  await page.evaluate(() => {
    ...
    const isConnectedStore = Object.values(stores).find((store: any) => 
      store && typeof store.set === 'function' && store.name === 'isConnected'
    ) as any;
...
async function disconnectWallet(page: Page) {
  ...
  await page.evaluate(() => {
    ...
    if (isConnectedStore) {
      ...
      window.dispatchEvent(new CustomEvent('appkit:disconnect', {
...
test.describe('Wallet Connection Flow', () => {
  ...
  test('should show connect wallet button on landing page', async ({ page }) => {
    ...
    const connectButton = page.locator('section').filter({ hasText: 'Live on Ethereum Mainnet' }).getByRole('button', { name: /connect wallet/i });
