...
test.describe('Admin Market Creation', () => {
	...
	test('should attempt to navigate to the admin market creation page', async ({ page }) => {
		...
		console.log(
			'Verified redirection to home page when accessing admin/market without wallet connection'
		);
	});
	...
	async function fillTokenFields(page: any, tokenA: string, tokenB: string) {
		...
		const tokenAAddress =
			tokenAddresses[tokenA as keyof typeof tokenAddresses] || tokenAddresses['ETH'];
		const tokenBAddress =
			tokenAddresses[tokenB as keyof typeof tokenAddresses] || tokenAddresses['USDC'];
		...
		try {
			...
			await page.evaluate(
				(tokens: { tokenA: string; tokenB: string }) => {
					...
					const tokenAInput =
						document.querySelector('#tokenA_address') ||
					...
					const tokenBInput =
						document.querySelector('#tokenB_address') ||
		...
		try {
			...
			const tokenASelectors = [
				'#tokenA_address',
				'[name="tokenA_address"]',
				'[placeholder*="Token A"]'
			];
			const tokenBSelectors = [
				'#tokenB_address',
				'[name="tokenB_address"]',
				'[placeholder*="Token B"]'
			];
	...
	async function waitForSubmissionResponse(page: Page): Promise<boolean> {
		const successSelector =
			'.bg-green-100, .success-message, [data-testid="success-message"], .notification-success, .alert-success';
		const errorSelector =
			'.bg-red-100, .error-message, [data-testid="error-message"], .notification-error, .alert-danger';
	...
	test('should interact with market creation form', async ({ page }) => {
		...
		const submitButton = page
			.locator('button:has-text("Create"), button:has-text("Submit"), button[type="submit"]')
			.first();
