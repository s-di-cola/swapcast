<script lang="ts">
	...
	const predictionHistory = [
		{
			id: 1,
			timestamp: '2023-11-10 10:00',
			marketName: 'ETH Price',
			assetPair: 'ETH/USDC',
			predictedDirection: 'Above',
			predictedPrice: '$2400',
		...
		{
			id: 2,
			timestamp: '2023-11-09 14:30',
			marketName: 'ETH Price',
			assetPair: 'ETH/USDC',
			predictedDirection: 'Below',
			predictedPrice: '$2300',
			stakedAmount: '1.2 ETH',
			outcome: 'Lost'
		},
		{
			id: 3,
			timestamp: '2023-11-12 09:00',
			marketName: 'BTC Target',
			assetPair: 'BTC/USDT',
			predictedDirection: 'Above',
...
<div class="min-h-screen bg-gray-50 text-gray-800">
	<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
		...
		<div class="mb-8">
			...
			<p class="mt-1 text-gray-600">View your predictions and market performance</p>
		</div>
					...
					{:else}
						<div class="grid grid-cols-1 gap-4 p-4 md:grid-cols-2">
							{#each markets.slice(0, 4) as market (market.id)}
								<button
									type="button"
									class="w-full cursor-pointer rounded-lg border border-gray-100 p-4 text-left transition-shadow hover:shadow-md"
									on:click={() => openMarketDetails(market.id)}
								>
									<div class="mb-2 flex items-start justify-between">
										<h3 class="font-medium text-gray-900">{market.name}</h3>
										<span
											class="rounded-full px-2 py-1 text-xs font-medium {market.resolved
									...
									<p class="mb-3 text-sm text-gray-600">{market.assetPair}</p>
									<div class="flex justify-between text-sm">
										<span class="text-gray-500">Threshold: ${market.priceThreshold}</span>
										<span class="text-gray-500"
											>Expires: {new Date(market.expirationTime * 1000).toLocaleDateString()}</span
										>
									</div>
								</button>
							{/each}
				...
				<div class="rounded-lg border border-gray-100 bg-white p-6 shadow-sm">
					...
					<div class="relative h-64">
						...
						<svg class="h-full w-full" viewBox="0 0 800 300" preserveAspectRatio="none">
							...
							<path d="{pricePath} L800,300 L0,300 Z" fill="url(#gradient)" opacity="0.2" />
				...
				<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
						...
						<div class="flex flex-col items-center justify-center">
							<div class="relative h-36 w-36">
								<svg viewBox="0 0 36 36" class="h-full w-full">
									...
									<path
										...
										stroke-width="2.17"
										stroke-linecap="round"
										stroke-dasharray="{(totalBullWeight / (totalBullWeight + totalBearWeight)) *
											100}, 100"
										d="M18 2.0845
                         ...
                         a 15.9155 15.9155 0 0 1 0 -31.831"
									...
									<text
										x="18"
										y="20.35"
										class="fill-current text-[5px] font-bold text-gray-700"
										text-anchor="middle"
										>{((totalBullWeight / (totalBullWeight + totalBearWeight)) * 100).toFixed(1)}%
										Bullish</text
									>
								</svg>
							</div>
					...
					<div class="rounded-lg border border-gray-100 bg-white p-6 shadow-sm">
						...
						<div class="flex flex-col items-center justify-center">
							...
							<p class="mt-1 text-sm text-gray-500">Based on last 20 predictions</p>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="mt-8 overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm">
			<div class="border-b border-gray-100 p-6">
				<div class="flex items-center justify-between">
				...
				<table class="min-w-full divide-y divide-gray-100">
					<thead>
						<tr>
							...
							>
							<th
								class="bg-gray-50 px-4 py-3 text-center text-xs font-medium tracking-wider text-gray-500 uppercase"
								>Status</th
							>
							<th
								class="bg-gray-50 px-4 py-3 text-center text-xs font-medium tracking-wider text-gray-500 uppercase"
						...
						{#each predictionHistory as prediction (prediction.id)}
							<tr class="transition-colors hover:bg-gray-50">
								...
								<td class="px-4 py-3 text-center text-sm whitespace-nowrap">
									...
									{:else}
										<span
											class="rounded-full bg-gray-50 px-2 py-1 text-xs font-medium text-gray-700"
											>Pending</span
										>
									{/if}
								</td>
