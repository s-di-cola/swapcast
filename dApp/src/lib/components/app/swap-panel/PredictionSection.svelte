<script lang="ts">
  ...
  let rewardAnalysis = $derived(() => {
    ...
    if (sideRatio > 2) riskLevel = 'high'; // Your side is crowded
    ...
    const crowdedSide = totalBullWeight > totalBearWeight * 1.5 ? 'bull' : 
                       totalBearWeight > totalBullWeight * 1.5 ? 'bear' : null;
...
<div class="space-y-4 rounded-xl border border-blue-100 bg-gradient-to-br from-blue-50/50 to-indigo-50/50 p-5">
  <!-- Prediction Header -->
    ...
    <div>
      ...
      <div class="text-sm text-gray-600 space-y-1">
        <div class="flex items-center gap-2">
          <span class="inline-flex items-center gap-1 rounded-md bg-orange-50 px-2 py-0.5 text-xs font-medium text-orange-700 border border-orange-200">
        ...
        <div class="text-xs text-gray-500 ml-2">
          (1% of your {payAmount.toFixed(6)} {payToken?.symbol} swap)
        </div>
      </div>
    </div>
    <button 
  ...
  <div class="mt-4 space-y-4">
    ...
    <div class="grid grid-cols-2 gap-3">
      <button
        ...
        class="flex flex-col items-center justify-center rounded-xl border-2 border-gray-200 bg-white p-4 text-center transition-all hover:border-emerald-300 hover:bg-emerald-50/50 hover:shadow-sm relative"
      >
        ...
        {#if predictionSide === 'above_target'}
          <div class="absolute -top-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-emerald-500 text-white">
            <Check class="h-4 w-4" />
          </div>
        {/if}
      </button>
      ...
      <button
        ...
        class="flex flex-col items-center justify-center rounded-xl border-2 border-gray-200 bg-white p-4 text-center transition-all hover:border-red-300 hover:bg-red-50/50 hover:shadow-sm relative"
      >
        ...
        {#if predictionSide === 'below_target'}
          <div class="absolute -top-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-white">
            <Check class="h-4 w-4" />
          </div>
        {/if}
      </button>
    ...
    <div class="space-y-3">
      <div class="space-y-2">
        <label for="targetPriceInput" class="text-sm font-medium text-gray-700">Target price in 24h</label>
        <div class="relative">
          ...
          <input
            ...
            class="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-center text-sm font-medium transition-all focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
  ...
  {#if predictionStakeAmount > 0 && (predictionSide === 'above_target' || predictionSide === 'below_target')}
    <div class="mt-4 space-y-3">
      <!-- Main Reward Display -->
      <div class="relative overflow-hidden rounded-xl border border-emerald-200 bg-gradient-to-br from-emerald-50 via-emerald-50/80 to-green-50 p-4 shadow-sm">
        <div class="relative z-10">
          ...
          <div class="mt-3">
            <div class="flex items-center justify-between mb-1">
              ...
              <span class="text-xs font-medium text-emerald-800">{rewardAnalysis().poolShare.toFixed(1)}%</span>
            </div>
            <div class="h-2 bg-emerald-100 rounded-full overflow-hidden">
              <div 
                class="h-full bg-gradient-to-r from-emerald-500 to-green-500 rounded-full transition-all duration-500 ease-out"
        ...
        <div class="absolute top-0 right-0 w-20 h-20 bg-emerald-200/30 rounded-full -translate-y-10 translate-x-10"></div>
        <div class="absolute bottom-0 left-0 w-16 h-16 bg-green-200/20 rounded-full translate-y-8 -translate-x-8"></div>
      </div>
        ...
        <div class="flex items-center justify-between mb-2">
          ...
          {#if rewardAnalysis().isEarlyPosition}
            <div class="flex items-center gap-1">
              ...
              <span class="text-xs font-medium text-orange-700">Early Position +{rewardAnalysis().timingBonus}%</span>
            </div>
          {/if}
        </div>
      ...
      {#if rewardAnalysis().crowdedSide || rewardAnalysis().isEarlyPosition || rewardAnalysis().riskLevel === 'low'}
        <div class="rounded-lg border border-blue-200 bg-blue-50/50 p-3">
          <div class="flex items-start gap-2">
            <div class="h-4 w-4 rounded-full bg-blue-500 flex items-center justify-center mt-0.5">
              <span class="text-xs text-white font-bold">💡</span>
            </div>
            <div class="flex-1 text-xs text-blue-800">
              {#if rewardAnalysis().riskLevel === 'low'}
                <span class="font-medium">Great odds!</span> You're betting on the underdog side - higher potential rewards if you're right.
              {:else if rewardAnalysis().isEarlyPosition}
                <span class="font-medium">Early bird bonus!</span> Small pool means your prediction has high impact and better rewards.
              {:else if rewardAnalysis().crowdedSide === 'bull' && predictionSide === 'below_target'}
                <span class="font-medium">Smart contrarian play!</span> Most people are bullish - bearish side has better odds.
              {:else if rewardAnalysis().crowdedSide === 'bear' && predictionSide === 'above_target'}
                <span class="font-medium">Against the crowd!</span> Most people are bearish - bullish side could pay big.
              {:else}
                <span class="font-medium">Balanced market!</span> Both sides have decent participation - fair odds.
              {/if}
            </div>
          </div>
        </div>
