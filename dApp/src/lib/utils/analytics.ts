...
export function processDailyAnalytics(data: any): DailyAnalytics[] {
  ...
  markets.forEach((market: any) => {
    ...
    console.log(`Added market ${market.id}, total markets now: ${dailyStats[todayStr].marketsCreated}`);
  ...
  console.log(`After processing markets, today's stats: marketsCreated=${dailyStats[todayStr].marketsCreated}`);
...
export function getLastNDaysData(data: DailyAnalytics[], days: number): DailyAnalytics[] {
  ...
  console.log('Final result after processing:', result.map(item => ({ 
    date: item.date, 
 ...
 */
export function getDateRangeForAnalytics(days: number = 30): { startTimestamp: string, endTimestamp: string } {
