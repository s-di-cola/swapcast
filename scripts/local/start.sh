...
deploy_contracts() {
  ...
  # Create a file to store all contract addresses
  ADDRESSES_FILE="$LOG_DIR/contract_addresses.env"
  echo "# Contract addresses deployed at $(date)" > $ADDRESSES_FILE
  ...
  # Save all addresses to the file
  [ -n "$SWAP_CAST_NFT" ] && echo "SwapCastNFT_ADDRESS=$SWAP_CAST_NFT" >> $ADDRESSES_FILE
  [ -n "$TREASURY" ] && echo "Treasury_ADDRESS=$TREASURY" >> $ADDRESSES_FILE
  [ -n "$PREDICTION_MANAGER" ] && echo "PredictionManager_ADDRESS=$PREDICTION_MANAGER" >> $ADDRESSES_FILE
  [ -n "$ORACLE_RESOLVER" ] && echo "OracleResolver_ADDRESS=$ORACLE_RESOLVER" >> $ADDRESSES_FILE
  [ -n "$REWARD_DISTRIBUTOR" ] && echo "RewardDistributor_ADDRESS=$REWARD_DISTRIBUTOR" >> $ADDRESSES_FILE
  
  # Set the PREDICTION_MANAGER_ADDRESS variable for use in the rest of the script
...
setup_subgraph() {
  ...
  echo "Updating subgraph configuration with contract address: $PREDICTION_MANAGER_ADDRESS"
  cd $SUBGRAPH_DIR
...
print_summary() {
  ...
  echo -e "Contract addresses are saved in: ${YELLOW}$LOG_DIR/contract_addresses.env${NC}"
